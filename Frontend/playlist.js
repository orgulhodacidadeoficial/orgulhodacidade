...
(function(){
    ...
    function handleMusicEnded() {
        if (autoPlayEnabled && currentPlayingIndex !== -1) {
            ...
            if (nextIndex < playlistData.length) {
                ...
                renderPlaylist(); // Re-renderiza para mostrar o botão de repetir
            }
        }
    }
    ...
    function handleAddMusic(e) {
        ...
        uploadAudioToServer(file).then(serverUrl => {
            ...
            // Toca a música automaticamente quando adicionada
            const audioPlayer = document.getElementById('audioPlayer');
            if (audioPlayer && musicData.blobUrl) {
                playMusicWithBlob(audioPlayer, musicData, playlistData.length - 1);
            }
        ...
        if (music && musicIndex !== -1) {
            ...
            if (audioPlayer) {
                ...
                if (window.Player && window.Player.getPlaylist) {
                    ...
                    const mainTrackIndex = mainPlaylist.findIndex(t => t.title === music.title);
                    
                    if (mainTrackIndex !== -1) {
                        // É uma música do player principal - usa a função do player
                        window.Player.loadTrack(mainTrackIndex);
                        window.Player.play();
                        currentPlayingIndex = musicIndex;
                        renderPlaylist();
                        return;
    ...
    function handlePlayNext() {
        const nextIndex = currentPlayingIndex + 1;
        if (nextIndex >= playlistData.length) {
            showNotification('⏹️ Você está na última música', 'info');
            return;
        }
    ...
    // Repete a playlist do início
    function handleRepeatPlaylist() {
        if (playlistData.length === 0) {
            showNotification('❌ Sua playlist está vazia', 'warning');
            return;
        }
        ...
        showNotification('🔄 Repetindo a playlist do início', 'success');
    }

    // Faz scroll até a música atual
    function scrollToCurrentTrack() {
        setTimeout(() => {
            ...
            const nextPlaySections = playlistContainer.querySelectorAll('.next-play-section');
            
            // Ajusta índice levando em conta as seções next-play
            const totalItems = items.length + nextPlaySections.length;
            const itemsBeforeNextPlay = nextPlaySections.length > 0 ? nextPlaySections[0].compareDocumentPosition(items[0]) : null;
            
            if (items[currentPlayingIndex]) {
                const item = items[currentPlayingIndex];
    ...
    function renderPlaylist() {
        ...
        // Cria HTML com seção de "Next Play"
        let html = '';
        
        // Seção Next Play (próxima música) ou Repetir Playlist
            ...
            if (nextIndex < playlistData.length) {
                ...
                html += `
                    <div class="next-play-section">
                        <div class="next-play-header">
                            <i class="fas fa-forward"></i> Próxima
                        </div>
                        <div class="next-play-item">
                            <div class="next-play-info">
                                <div class="next-play-title">${escapeHtml(nextMusic.title)}</div>
                                <div class="next-play-artist">${escapeHtml(nextMusic.artist)}</div>
                            </div>
                            <button class="btn-play-next" onclick="window.PlaylistManager.playNext()">
                                <i class="fas fa-play"></i> Pular
                            </button>
                        </div>
                    </div>
                `;
            } else if (currentPlayingIndex >= 0 && playlistData.length > 0) {
                // Última música - mostrar botão de repetir
                html += `
                    <div class="next-play-section next-play-repeat">
                        <div class="next-play-header">
                            <i class="fas fa-redo"></i> Fim da Playlist
                        </div>
                        <div class="next-play-item">
                            <div class="next-play-info">
                                <div class="next-play-title">Você chegou ao final da sua playlist</div>
                                <div class="next-play-artist">Clique para repetir do início</div>
                            </div>
                            <button class="btn-play-next btn-repeat" onclick="window.PlaylistManager.repeatPlaylist()">
    ...
    window.PlaylistManager = {
        ...
        repeatPlaylist: handleRepeatPlaylist,
